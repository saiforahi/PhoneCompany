...
void KomKothaKoiben::customer_dequeueCall(string givenNID, PhoneNumber givenPhoneNumber,Call & callToDequeue)
{
	if (!customerList.empty())
	{
		auto marker = customerList.begin();
		for (; marker != customerList.end(); marker++)
		{
			if (marker->get_NID() == givenNID)
			{
				marker->dequeue_call(givenPhoneNumber, callToDequeue);
				break;
			}
		}
	}
